...
func (chain *chainImpl) processMessagesToBlocks() ([]uint64, error) {
	...
	for {
		...
		case in, ok := <-chain.channelConsumer.Messages():
			...
			case *ab.KafkaMessage_TimeToCut://TTC,打包现有的一批消息 生成区块
			...
			case *ab.KafkaMessage_Regular://处理Fabric相关消息，包括配置更新，应用通道交易等
		...
		case <-timer: //定期发出TimeToCut消息到kafka
